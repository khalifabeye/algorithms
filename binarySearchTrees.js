import visualizeBST from "./visualizeTree.js";
...
class BinarySearchTrees {
  ...
  dfsPreorder() {
    ...
    let result = [];
      // while there is a left and right child
      while (stack.length) {
        current = stack.pop();
        result.push(current.value);
        // logPop.push(stack.pop().value) //HACK
        if (current.rightChild) stack.push(current.rightChild);
        if (current.leftChild) stack.push(current.leftChild);
        // stack.forEach(el => console.log(el.value));
      }
  ...
  dfsPostorder(node, result = []) {
    ...
    // while there is elements in the stack
    this.dfsPostorder(node.rightChild, result);
    this.dfsPostorder(node.leftChild, result);
    result.push(node.value);
    return result;
  }
